...
const OcrScreen = () => {
  ...
  const [ocruploadstore, setOcrUploadStore] = useState("deepanshu_test");
  ...
  const handleSearchVendor = debounce(async query => {
    ...
    try {
      ...
      const res = await axios.post(
        // `http://192.168.1.52:3006/api/searchvendor`,
        API_ENDPOINTS.SEARCHVENDOR,
        {
          q: query,
        },
      );
      ...
      if (Array.isArray(res.data.results)) {
        console.log('Vendor search results:', res.data.results);
        setSearchResults(res.data.results);
      } else {
        console.warn('Unexpected vendor data format:', res.data);
        setSearchResults([]);
      }
    } catch (err) {
