...
export default function MixMatchScreen() {
  ...
  const loadPromotions = async () => {
    try {
      ...
      console.log('mix match promotions response:', data);
      setRows(Array.isArray(data) ? data : []);
      setDaysList(Array.isArray(days) ? days : []);
      if (Array.isArray(data)) {
        console.log('mix match days_of_week_ids (load):', data.map((d) => d?.days_of_week_ids));
      }
    } catch (e) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((item) => {
      ...
      const startVal = item?.start_date || '';
      const endVal = item?.end_date || '';
      const startOk = !startFilter || (startVal && startVal >= startFilter);
      const endOk = !endFilter || (endVal && endVal <= endFilter);
      ...
      <View style={styles.panelInner}>
        <View style={styles.searchCard}>
          ...
          <View style={styles.searchRow}>
            <TextInput
              style={[styles.searchInput, styles.searchHalf]}
