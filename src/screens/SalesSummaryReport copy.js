...
import { SaleSummaryPaymentType,SaleSummaryCashReport,SaleSummaryRefundReport,SaleSummaryTaxReport,SaleSummaryDepartmentAllianceReport } from '../functions/reports/pos_reports';
...
const fmtLocal = (d) => {
  ...
  return `${y}-${m}-${day} ${hh}:${mm}:${ss}`;
    ...
    const [pickerVisible, setPickerVisible] = useState(false);
     const [paymentTypeReport, setPaymentTypeReport] = useState(false);
      const [cashInOutReport, setCashInOutReport] = useState(false);
      const [refundReport, setRefundReport] = useState(false);
     const [taxReport, setTaxReport] = useState(false);
  ...
  const handleSaleSummaryReport = async () => {
    try {
      //payment type report
      const paymentreport = await SaleSummaryPaymentType(fmtLocal(range.start), fmtLocal(range.end));
      console.log('PaymentTypeReport (screen) length:', paymentreport.length);
      setPaymentTypeReport(paymentreport);

      //cash in out report
      const cashreport = await SaleSummaryCashReport(fmtLocal(range.start), fmtLocal(range.end));
      console.log("cash report:",cashreport);
      setCashInOutReport(cashreport);
// refund report 
      const refundreport = await SaleSummaryRefundReport(fmtLocal(range.start), fmtLocal(range.end));
      console.log("refund report:",refundreport);
      setRefundReport(refundreport);

      // tax report 
      const gettaxreport = await SaleSummaryTaxReport(fmtLocal(range.start), fmtLocal(range.end));
      console.log("tax report:",gettaxreport);
      setTaxReport(gettaxreport);
      
    // fetch department sales report alliance
      const getDepartmentreport = await SaleSummaryDepartmentAllianceReport(fmtLocal(range.start), fmtLocal(range.end));
      console.log("department report:",getDepartmentreport);
      setDepartmentReport(getDepartmentreport);
    

    } catch (e) {
      console.log('error:', e?.message || e);
      setPaymentTypeReport([]); // keep UI stable
    }
  };
    useEffect(() => {
    handleSaleSummaryReport();
    }, []);
...
<ReportTabs
  ...
  tabs={[
    { key: "POS Payment Collection", component: PosPaymentCollectionTab },
    { key: "Cash In & Out", component: CashInOutTab },
    // later: { key: "Cash Tender Details", component: CashTenderDetailsTab },
   { key: "Tax Report", component: TaxReportTab },
    {key: "Refund Report",component: RefundReportTab },
   {key: "Department Wise Report",component: DepartmentWiseReportTab },
  ]}
  apiData={{
